...
if ARGV.size != 4
	...
	paramator_u = ARGV[1].to_f
	paramator_ramda = 0
	# 文書フレーム化単語数
	frame_num = ARGV[2].to_f
       n_best = 1
end
## ここまで設定項目
...
### tf_hash_temp: TFのハッシュに入れるためのとりあえず配列
 tf_hash_line= Array.new
 mor_num_arr = Array.new
 tf_num = 0
 tf_hash_line[tf_num] = Hash.new
## nbestファイルを読み込む
 for i in 1 .. n_best 
### res: ファイルを読み込みMeCabで形態素解析した結果
### 文字コード：UTF-8
### 改行コード：Linux(LF)
  res = mcb.parse(NKF.nkf("-w -m0 -Lu", File.read(dir + filelist[file_num] +"/"+ i.to_s + "best.txt")))
	  ...
	  str = line.chomp.split(",")
	    ...
	    if str[1] == "名詞"
	     mor_num = mor_num + 1.0/i.to_f
 	     mor_num_all += 1.0/i.to_f
	      # tf_hashに既に格納されているか
	      #  格納されていなければ 1 (else部分)
	      #  格納されている時は +1
	      if tf_hash_line[tf_num].key?(str[0])
		tf_hash_line[tf_num][str[0]] += 1.0/i.to_f
	      else
		tf_hash_line[tf_num][str[0]] = 1.0/i.to_f
	      end
	      if mor_num == frame_num
		mor_num_arr << mor_num
		mor_num = 0
		tf_num += 1
		tf_hash_line[tf_num] = Hash.new
...
## 一つの文書の処理が終わったらtf_resultに結果をプッシュ 
 tf_result << tf_hash_line.to_a
 mor_num_line << mor_num_arr
end
tf_hash_line = Hash.new
...
## tfidf_hash: TF-IDFのハッシュ
  tfidf_hash_line = Array.new
## tfidf_nor: 文書の長さ統一のためのノルム
  tfidf_nor = 0.0
## 文書内の単語毎にtfidfを求める。
  for i in 0 .. tf_result[doc_num].size-1
  tfidf_hash_line[i] = Hash.new
   for word in tf_result[doc_num][i]
     tfidf_hash_line[i][word[0]] = word[1].to_f / mor_num_line[doc_num][i].to_f   
...
#	lex_temp[page_num][lex[0]] = lex_temp[page_num][lex[0]] * idf_hash[lex[0]]
#	lex_temp[page_num][lex[0]] = (Math::log(lex_temp[page_num][lex[0]])+1.0)*idf_hash[lex[0]]
	tfidf_nor += lex_temp[page_num][lex[0]] * lex_temp[page_num][lex[0]]
  end
#### 重みをかける
  for lex in lex_temp[page_num]
#	lex_temp[page_num][lex[0]] /= Math.sqrt(tfidf_nor) # ノルム重み
  end
 end
####
...
for query_num in 0..belonged_query_vector.size-1
  for word in belonged_query_vector[query_num]
	...
	query_vector[word[0]] += word[1].to_f 
      else
	query_vector[word[0]] = word[1].to_f
     end
  end
...
### 各質問クエリの名詞毎に計算
   for word in origin_query_vector[query_num]
    for partation in 0 .. doc_vector[doc_num].size-1
### 重みを付与するための分母
   smooth = mor_num_line[doc_num][partation] + paramator_u
#### 検索対象文書の名詞情報
     if doc_vector[doc_num][partation].key?(word[0])
	pwd = mor_num_line[doc_num][partation].to_f * doc_vector[doc_num][partation][word[0]].to_f / smooth
     else
	pwd = 0
     end
#### 文書コレクションを用いた重み
...
### それらをクエリとして計算
     if query_vector.key?(word[0])
      temp += word[1]*Math::log((1-paramator_ramda)*(pwd+pwcol) + paramator_ramda * query_vector[word[0]]  ) 
     else
      temp += word[1]*Math::log((1-paramator_ramda)*(pwd+pwcol) )
     end
    end 
   end
###
# 尤度を類似度として記憶
   simirality_point[filelist[doc_num]] =  doc_vector[doc_num].size
###################
